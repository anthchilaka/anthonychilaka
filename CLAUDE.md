...
This project is mid-migration from SPA to MPA. Two files take precedence over generic guidance elsewhere in this file wherever they conflict:

- **`blueprint.md`** — the current build plan and task state. Check this at the start of every session before making changes.
- **`.claude/skills/website-build/SKILL.md`** — project-specific architecture, file structure, coding conventions, and safety rules for the MPA build.
...
| SEO audit or GEO content | `/seo-geo` (not yet pushed to this repo) |
| GTM event implementation | `/gtm-datalayer` (not yet pushed to this repo) |
| BigQuery query or pipeline debug | `/bigquery-gold` (not yet pushed to this repo) |
| Claude Code setup or MCP config | `/claude-code-setup` (not yet pushed to this repo) |
...
| BigQuery | NL → SQL on Gold Layer. IAM granted May 2026 — unconfirmed as of Aug 2026, contradicts a desktop-session finding of no BigQuery access tool; verify in this Studio session. |
